...
4. `locations/{loc}`: readable by active users at that location. Writable with `location.manage`, except `nextDeviceNo`: no one may change it except `device.register` at that location, by exactly +1, in the same transaction that creates `devices/D{new value}`. A new location is created with `nextDeviceNo == 0`.
   ...
   - **create:** `device.register` at `loc`, only if it doesn't exist, the ID is `D` plus the new `nextDeviceNo` (2 digits), and `getAfter(location).nextDeviceNo == resource.nextDeviceNo + 1`.
   - **update `lastBillSeq`, `lastMovementSeq`, `lastReturnSeq`:** any active user at `loc` with `bill.create`, `stock.move` or `return.create` respectively; each may only go up.
...
5. `bills`: **create-only** with `!exists`, with `canAt('bill.create', loc)`. No delete. Two kinds of update, and nothing else may change:
   ...
   - **(b) return:** only `returnedQty` and `lastReturnId` change, with `return.create`. Requires `resource.data.status == 'COMPLETED'`, `lastReturnId` names a return that did not exist before the batch (`!exists`) and does after (`existsAfter`), and for each key in `returnedQty` (unrolled): the value only grows, and `returnedQty[p] <= soldQty[p]`.
...
7. `returns` and `movements`: create-only with `!exists`, with the matching permission at the location (`return.create`; `stock.move`, `stock.adjust`, or `bill.create`/`bill.cancel` for SALE/CANCEL movements). `createdBy == request.auth.uid` and `lines.size() <= 20` on both.
...
8. `stock/{itemKey}`: batches never write a literal `qty`; it is always `increment(delta)` through `set(merge)`, which also creates the doc on first use (D-005). Create and update may change only `kind`, `refId`, `name`, `unit` (these four only on create), `qty`, `lastMovementId`, `updatedAt`, and `lowThreshold` (which needs `stock.threshold`). Any change to `qty` requires that the movement named in `request.resource.data.lastMovementId` did **not** exist before the batch and **does** after (`!exists` and `existsAfter`).
