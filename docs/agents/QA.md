...
- **Running device scenarios:** from `apps/pos`, with the Firebase emulator running and an Android emulator started: `flutter test integration_test --dart-define=USE_EMULATOR=true`. The Android emulator reaches the host's Firebase emulator at `10.0.2.2`.
