...
|---|---|---|
| **Dart** | `test/e2e/test/`, `dart test`, no emulator | `nexus_core`, plus the backend's D-027 JSON plan fixtures in `firebase/test/fixtures/plans/<name>.json` once they exist (this package stays pure Dart) |
...
- Expected: A's return lands (₹2). B's whole return batch is rejected by rule #5(b) (its `prevReturnId` is null, but the bill's `lastReturnId` is now A's return) and shows in B's `SyncService.errors`: no return doc, no RETURN movement, no stock or summary change. The redone return has `prevReturnId` = A's return and refunds ₹1; the last unit ₹2. Σ `refundTotal` = ₹5 (D-024 d), and the bill's returns chain through `prevReturnId` back to null.
...
- Steps: random bills (1–4 lines, and now and then 11–15 lines from a wider catalogue; paise prices, a free item, none / flat / % discounts up to 100%, 1–4 payments, cash tendered), same-day cancellations, full or partial returns of bills from any earlier day with 1–3 refund modes, and conflicting offline pairs (a stale cancel after a synced return, a stale return after a synced cancel, a stale return past `soldQty`, and a stale return within `soldQty` after a synced return, which is then redone from the fresh bill). Every batch goes through a model of rules #5(a), #5(b) (with `prevReturnId`) and #6; a rejected batch writes nothing and counts as a sync error. Summaries are built only from `SummaryDeltas`, both with `Summary.+` and as `increments()` field paths read back with `Summary.fromMap`.
- Expected, for every day and month at each location and for both combined: every field (counts, gross, discounts, round-off, net sales, returns, cancelled, `byMode`, `byProduct` qty and amount) equals the recomputation from the documents; `netRevenue` equals the money kept; Σ `byMode` equals `netRevenue`; Σ `byProduct.amount` equals the pre-round-off net; each monthly summary equals the sum of its days. Per bill: refunds never exceed its total, equal it after a full return, and follow D-024 (d) cumulatively; `returnedQty` matches its returns; per-line nets are the largest-remainder split. Per bill doc (D-029, D-030): at most 15 lines and 4 payments; `soldQty` holds each line's qty; `returnedQty` never holds a 0 and never exceeds `soldQty`; a CANCELLED bill has an empty `returnedQty`; `lastReturnId` names the bill's latest return, and its returns form one `prevReturnId` chain back to null. Every lost conflict is a sync error and left no return doc. A coverage test proves the seed reaches each edge case (zero-total bills, cap rejections, zero-refund returns, cross-month returns, refused next-day cancels, refused cancel after return, refused over-returns, refused returns on cancelled bills, bills over 10 lines and at exactly 15, a refused 16-line cart, and each of the four lost conflicts). Without the `prevReturnId` check in the model, the cumulative-refund assertions fail (checked by removing it).
...
**P-03 The property through the WritePlan fixtures.** Layer: Dart (`test/e2e` reads the D-027 JSON fixtures from `firebase/test/fixtures/plans/`). Waits: BE-10.
...
**A6-10 Restart while blocked (03-SYNC §6.4, QA-025).** At 5 h offline, blocked, kill and restart the app (still offline; Firebase Auth restores the session). Expected: still `BillingBlocked`, with the same `elapsed` (`lastSyncAt` is read back from the local store; restoring the session does not set it); an override still in force before the restart is still in force after it, until its original end. During an override the state is `NearLimit` and `billingStopsIn` counts down to the override's end. Also reboot the phone in QA-8.
...
**S5-6 Rename after first use (QA-023).** Layer: Device and Rules. Waits: BE-4, BE-10.
- Steps: FG item X has a stock doc at PTB; the Admin renames product X; the POS (with the new name cached) sells X; also rename a raw material and STOCK_IN it. Then a device that still has the old name cached sells X.
...
| R-4d | `locations/{loc}/devices/*` | create by `device.register` at `loc` at `D{nextDeviceNo+1}` while not existing, with `getAfter(location).nextDeviceNo == get(location).nextDeviceNo + 1`, up to `D99`; `lastBillSeq` / `lastMovementSeq` / `lastReturnSeq` raised by a user with `bill.create` / `stock.move` or `stock.adjust` / `return.create`; `lastSeenAt` by any active user at `loc`; `label` / `retired` with `location.manage`; read with `device.register` or `location.manage` | create at an existing code; create at a code that isn't the new `nextDeviceNo`; create at MNJ as SM @ PTB; any `last*Seq` lowered; `label` or `retired` by an SM; any write by `uid-sm-off`; a registration past `D99`; with a fixture role that has only one of the permissions (e.g. `stock.adjust` without `stock.move`), each counter needs its own permission | BE-2 |
...
| R-14 | Rules budgets (D-030, CR-001) | a 15-line bill with 4 payments, a return update that brings all 15 products into `returnedQty` (with its 4-refund return and `prevReturnId`), and a 20-line PRODUCE are each accepted on the emulator, within 20 access calls per batch and 1000 expressions per doc | — | BE-6 |
...
| 01 acceptance #4 | A4-1..A4-7 |
| 01 acceptance #5 | P-01..P-05, M-1..M-4 |
